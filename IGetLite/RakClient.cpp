...
bool RakClient::ConnectPeer( const string & peerHost, unsigned short peerPort, const string & passwd)
{
	return m_pPeer->Connect( peerHost.c_str(), peerPort, passwd.c_str(), (int)passwd.size() );
}
...
int RakClient::OnConnectionLost(Packet* p)
{
...
SystemAddress RakClient::GetServerIPAddr()
{
